 ...
 * @LastEditors: 曹俊
 * @LastEditTime: 2023-03-16 14:44:06
-->
...
- 前端常见的安全问题包括：
  ...
  - XSS攻击对策：对用户输入进行过滤和转义，过滤掉危险的字符和标签;使用 HttpOnly 属性限制 cookie 只能在 HTTP 请求中传输，防止 cookie 被窃取;使用 CSP（Content Security Policy）限制网页中脚本的执行，只允许加载指定的脚本资源;对 URL 参数进行校验和过滤，防止恶意脚本的注入。
